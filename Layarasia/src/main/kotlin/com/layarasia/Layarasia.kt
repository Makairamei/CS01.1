...
val episodes = episodeElements
    ...
    newMovieLoadResponse(title, url, TvType.Movie, episodes.firstOrNull()?.data ?: url) {
        ...
        document.selectFirst(cfg.playerSelector)
            ?.let { el ->
                val primary = el.attr(cfg.playerAttr)
                val fallback = if (primary.isNullOrBlank()) el.attr(cfg.playerFallbackAttr) else primary
                if (!fallback.isNullOrBlank()) {
                    loadExtractor(httpsify(fallback), data, subtitleCallback, callback)
                }
            }
    ...
    private fun Element?.getIframeAttr(primary: String, fallback: String): String? {
        return this?.attr(primary).takeIf { it?.isNotEmpty() == true } ?: this?.attr(fallback)
    }
